  ...
  case "$OS" in
    ubuntu|debian|raspbian)
      ...
      if ! dpkg -s fd-find >/dev/null 2>&1; then
        MISSING_DEPS="fd-find $MISSING_DEPS"
      fi

      if ! dpkg -s fzf >/dev/null 2>&1; then
        MISSING_DEPS="fzf $MISSING_DEPS"
      fi
      ;;
    ...
    arch|manjaro)
      ...
      if ! pacman -Qi fd >/dev/null 2>&1; then
        MISSING_DEPS="fd $MISSING_DEPS"
      fi
    ...
    *)
      ...
      echo "Please install git, pkg-config, ncurses, build tools, fd, and fzf manually."
      exit 1
      ;;
  esac
